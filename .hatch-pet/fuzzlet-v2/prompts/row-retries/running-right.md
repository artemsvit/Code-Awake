Create Codex pet row `running-right` for `fuzzlet`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Preserve the existing Fuzzlet identity: a cool fuzzy 3D plush-like single soft body, big expressive eyes, consistent blue-violet fur palette and compact friendly silhouette. Upgrade only by adding coherent viewer-coordinate look directions.. Preserve silhouette, face, palette, material, proportions, markings, and props.
