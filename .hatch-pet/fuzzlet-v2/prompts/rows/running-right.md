Create one horizontal animation strip for Codex pet `fuzzlet`, state `running-right`.
...
Identity: same pet in every frame: Preserve the existing Fuzzlet identity: a cool fuzzy 3D plush-like single soft body, big expressive eyes, consistent blue-violet fur palette and compact friendly silhouette. Upgrade only by adding coherent viewer-coordinate look directions.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `3d-toy`: Stylized 3D toy mascot with smooth rounded forms, simple materials, clear silhouette, and no photoreal complexity. User style notes: Match the existing approved atlas exactly: fuzzy 3D plush material, cool blue-violet fur, big expressive eyes, clean compact silhouette, no new props..
